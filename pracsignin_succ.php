			...
			<div class="table tab-content ">
				...
				<h2 style="color:white;">Account Details</h2>
					...
					<tbody>
						<tr>
							<td>Name</td>
							<td><?php $sqlquery="select username from customer_details where username='$name' and password='$pwd'";
						...
						<tr>
							...
							<td><?php $sqlquery="select reg_no from customer_details where username='$name' and password='$pwd'";
										$result=mysqli_query($con,$sqlquery);
										$rows=mysqli_fetch_assoc($result);
										echo $rows['reg_no']; 
										?>
						...
						<tr>
							<td>Phone Number</td>
							<td>
							<?php $sqlquery="select phone_no from customer_details where username='$name' and password='$pwd'";
										$result=mysqli_query($con,$sqlquery);
										$rows=mysqli_fetch_assoc($result);
										echo $rows['phone_no']; 
										?>
						...
						<tr>
							...
							<td><?php $sqlquery="select account_bal from customer_details where username='$name' and password='$pwd'";
										$result=mysqli_query($con,$sqlquery);
										$rows=mysqli_fetch_assoc($result);
										echo $rows['account_bal']; 
										?>
							</td>
						</tr>
...
<?php
	if(isset($_POST['send_money_btn']))
	{
		...
		$sqlquery11="select * from customer_details where username='$from'";
		$result11=mysqli_query($con,$sqlquery11);
		$rows11=mysqli_fetch_assoc($result11);
		$amount_sender=$rows11['account_bal']-$amount; 
		
		$sqlquery22="select * from customer_details where username='$to'";
		$result22=mysqli_query($con,$sqlquery22);
		$rows22=mysqli_fetch_assoc($result22);
		$amount_receiver=$rows22['account_bal']+$amount; 
		
		
		
		$query1="update customer_details set account_bal='$amount_receiver' where username='$to'";
		$result1=mysqli_query($con,$query1);
		
		$query2="update customer_details set account_bal='$amount_sender' where username='$from'";
